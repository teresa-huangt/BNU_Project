...
%         end
        ...
        %EEG = pop_eegchanoperator(EEG, [parentfolder 'chanops_reref_biveog.txt']);
          ...
          EEG = pop_eegchanoperator(EEG, [parentfolder 'chanops_reref_biveog_avg.txt']); %for  mean reference
%          EEG = pop_eegchanoperator(EEG, [parentfolder 'chanops_reref_biveog_exp3.txt']);
        ...
        fprintf('\n\n\n**** %s: Running BinLister ****\n\n\n', subject);  
        EEG  = pop_binlister( EEG , 'BDF', [parentfolder 'binlister_demo_2.txt'], 'IndexEL',  1, 'SendEL2', 'EEG', 'Voutput', 'EEG' ); %ddd与教程不同% GUI: 04-Aug-2016 16:37:14
...
%         EEG.setname = ['refAF34' EEG.setname '_epochs_noLpass']; % for exp3
        EEG.setname = ['refM_noLpass' EEG.setname '_epochs'];
        if (1)
            EEG = pop_saveset(EEG, 'filename', [EEG.setname '.set'], 'filepath', subjectfolder);
        end   
