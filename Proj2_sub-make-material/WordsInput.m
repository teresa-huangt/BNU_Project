function WordsInput(subID,command)
...
load( 'word20170802_amp.mat'); %补录修改过
...
switch command
    case 'demo' 
         load('word20170802_amp.mat');
        trialN = 5;
        %待改正：伪随机一个练习组：真假词都要练到...
    case 'run'
        trialN = N;
end
...
h_type = uicontrol ('style', 'text', 'fontsize', bigFont,...
    'Fontname', 'Crescent', 'String', ...
   '请输入拼音+声调','units', 'normalized', 'position', [.3 .6 .4 .2], ...
    'BackgroundColor', [0.8 0.8 0.8],'visible','off');
...
%         cue = [cue{1},cue{2},cue{3}];
    ...
    %%
      function evaluate(hObject, eventData)
        ...
        response{i} = S;
        isCorrect(i) = strcmp(response{i},idSpoken{ind,1});%1 if correct, 0 if not
        set(h_input,'String','');
