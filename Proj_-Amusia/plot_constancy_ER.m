function plot_constancy_ER(taskN,index) 
% 
% index:E,C,V,T,S
%
% NOTE:!!! 
...
switch index    
        case 'C' %mean
        structureDir = data2.(taskName).constancy_mean.C(:,[3 4]);
        titleName = ['TAIL-'  taskName '-C'];   
        labelXtick = {'SC', 'DC'};
        case 'V' 
        structureDir = data2.(taskName).constancy_mean.V(:,[3 4]);
        titleName = ['TAIL-'  taskName '-V'];
        labelXtick = {'SV', 'DV'};
        case 'L' 
        structureDir = data2.(taskName).constancy_mean.ear(:,[3 4]);
        titleName = ['TAIL-'  taskName '-L'];
        labelXtick = {'SL', 'DL'};
                case 'W' 
        structureDir = data2.(taskName).constancy_mean.W(:,[3 4]);
