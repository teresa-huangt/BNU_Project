function plot_phonemeLevel_CVT_RT(taskN,calIndex) 
...
if exist('N.mat', 'file')
    ...
    N.(taskName).CVTst = nanmean(data2.(taskName).CVT_mean.Nst);
    N.(taskName).CVTdt = nanmean(data2.(taskName).CVT_mean.Ndt);
save N N
...
switch calIndex
        ...
        labelY='mean RT (s)';
            % CVT
            structureDir_CVTst = data2.(taskName).CVT_mean.RTst;
            structureDir_CVTdt = data2.(taskName).CVT_mean.RTdt;      
            
    
        case 'median' % median
        labelY='median RT (s)';
            % CVT
            structureDir_CVTst = data2.(taskName).CVT_median.RTst;
            structureDir_CVTdt = data2.(taskName).CVT_median.RTdt;

end


% %% CVT (st£º'SCSV' ,'SCDV', 'DCSV', 'DCDV'; dt£º'SCSV','SCDV','DCSV','DCDV') figure RT 
